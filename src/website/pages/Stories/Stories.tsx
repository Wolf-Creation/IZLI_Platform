      ...
      <section style={{ background: SURFACE, borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 40px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, minHeight: 480 }}>
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '64px 64px 64px 0' }}>
            ...
            <h1 style={{ fontFamily: FONT_SERIF, fontSize: 48, fontWeight: 500, color: INDIGO, margin: 0, lineHeight: 1.1, marginBottom: 18 }}>Indigo as Memory</h1>
            <p style={{ fontSize: 15, color: TEXT_SEC, lineHeight: 1.75, margin: 0, marginBottom: 28, maxWidth: 400 }}>How a color became a carrier of culture — a deep dive into the history of indigo dyeing across North Africa and its presence in IZLI's work.</p>
            <div style={{ display: 'flex', gap: 12 }}>
              <button style={{ padding: '11px 24px', background: INDIGO, color: CREAM, border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: FONT_SANS }}>Read Story</button>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '48px 40px 80px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 36 }}>
          ...
          <div style={{ display: 'flex', gap: 4, background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 4 }}>
            {FILTERS.map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: '6px 16px', borderRadius: 9, border: 'none', background: filter === f ? INDIGO : 'transparent', color: filter === f ? CREAM : TEXT_SEC, fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: FONT_SANS }}>{f}</button>
            ))}
          </div>
        </div>
